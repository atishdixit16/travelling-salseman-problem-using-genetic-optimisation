...
randomTrials <- function(start_city, count = 100, citiesVector) {
	ind <- which(citiesVector==start_city)
	citiesVector <- citiesVector[-ind]
	trialStacks <- NULL
	for (i in 1:count)
		trialStacks <- rbind( trialStacks ,  c( start_city , sample(citiesVector) ) )
	trialStacks
}
...
GeneOpt <- function( func = pathDistance, distanceMatrix=distance, iterations=50, count=100, citiesVector, startCity, crossProb=0.8, mutate_prob=0.001 , display=TRUE , cordList) {
	n_cities <- length(citiesVector)
	trialStacks <- randomTrials(startCity ,count , citiesVector )
...
for(i in 1:clustNo) {
	...
	ind <- which(minDist==min(minDist))[1]
	closestCities1 <- c(closestCities1, citiesList[[i]][ind])
}
...
for (i in 1:clustNo) {
	Answer <- GeneOpt(func = pathDistance, distanceMatrix=distance, iterations=200, count=100, citiesList[[i]], closestCities1[i], crossProb=0.8, mutate_prob=0.001 , display=FALSE,a )
	clusterPaths[[i]] <- Answer$arg_min
	if (i%%2)
		mainPath <- c( mainPath, rev(clusterPaths[[i]]) )
	else
